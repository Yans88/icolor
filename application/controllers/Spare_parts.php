...
class Spare_parts extends MY_Controller {
	...
	public function index() {
		if(!$this->session->userdata('login') || !$this->session->userdata('spare_parts')){
			$this->no_akses();
			return false;
		}	
		$this->data['judul_browser'] = 'Spare Parts';
		$this->data['judul_utama'] = 'Spare Parts';
		$this->data['judul_sub'] = 'List';
		$this->data['tipe'] = 1;
		$this->data['tools'] = $this->access->readtable('spare_parts','',array('deleted_at'=>null))->result_array();
		$this->data['isi'] = $this->load->view('tools_sp/spare_parts_v', $this->data, TRUE);
	...
	public function del(){
		...
		$where = array(
			'id_spare_parts' => $_POST['id']
		);
		$data = array(
			'deleted_at'	=> $tgl,
			'deleted_by'	=> $this->session->userdata('operator_id')
		);
		echo $this->access->updatetable('spare_parts', $data, $where);
	}	
