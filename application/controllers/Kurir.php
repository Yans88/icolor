...
class Kurir extends MY_Controller {
	...
	public function index() {	
		if(!$this->session->userdata('login') || !$this->session->userdata('kurir')){
			$this->no_akses();
			return false;
		}
		$this->data['judul_browser'] = 'Kurir';
		$this->data['judul_utama'] = 'Kurir';
		$this->data['judul_sub'] = 'List';
		$select = array('kurir.*','store.nama as nama_store');
		$this->data['kurir'] = $this->access->readtable('kurir',$select,array('kurir.deleted_at'=>null),array('store' => 'store.id_op = kurir.id_store'),'','','LEFT')->result_array();
		$this->data['store'] = $this->access->readtable('store','',array('deleted_at'=>null))->result_array();	
		$this->data['isi'] = $this->load->view('kurir/kurir_v', $this->data, TRUE);
	...
	public function del(){
		...
		echo $this->access->updatetable('kurir', $data, $where);
	}
	...
	public function save(){
		...
		if($id > 0){
			...
			$this->access->updatetable('kurir', $simpan, $where);			
			$save = $id;
		}else{
			$simpan += array('created_at' => $tgl,'created_by' => $this->session->userdata('operator_id'));
			$save = $this->access->inserttable('kurir', $simpan);   
		}
