...
class Master_bank extends MY_Controller {
	...
	public function index() {
		if(!$this->session->userdata('login') || !$this->session->userdata('bank_icolor')){
			$this->no_akses();
			return false;
		}
		...
		$this->data['master_payment'] = $this->access->readtable('master_akun_bank','',array('deleted_at'=>null))->result_array();			
	...
	public function save(){		
		...
		if(!empty($id_bank)){			
			...
			$save = $this->access->updatetable('master_akun_bank', $data, $where);
		}else{
			$data += array('created_at' => date('Y-m-d H:i:s'));
			$save = $this->access->inserttable('master_akun_bank', $data);
		}		
		echo $save;	
	...
	public function del(){	
		...
		echo $this->access->updatetable('master_akun_bank', $data, $where);
	}	
