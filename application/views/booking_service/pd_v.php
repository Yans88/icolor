<style type="text/css">
	...
	.over_work{background-color: #FA5858 !important;}
	.until_over{background-color: #F7FE2E !important;}
</style>
<?php
$tanggal = date('Y-m');
...
?>
<div class="modal fade" role="dialog" id="confirm_diterima_cust">
          <div class="modal-dialog" style="width:400px">
            <div class="modal-content">
              ...
              <div class="modal-body">
				<h4 class="text-center">Apakah anda yakin <br/>paket ini telah diterima oleh customer ? </h4>
				<br/>
				<div class="form-group">
                  <label>Catatan</label><span class="label label-danger pull-right"></span>
                  <textarea class="form-control" name="catatan_terima_cust" id="catatan_terima_cust" placeholder="Catatan" rows=5></textarea>
                 
                </div>
              </div>
              <div class="modal-footer" style="margin-top:0px;">
                <button type="button" class="btn btn-danger" data-dismiss="modal">Close</button>               
                <button type="button" class="btn btn-success yes_diterima_cust">Ya, telah diterima customer</button>               
              </div>
            </div>
            <!-- /.modal-content -->
          </div>
          ...
          <div class="modal-dialog" style="width:600px;">
            <div class="modal-content">
              <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">×</span></button>
                <h4 class="modal-title">Info Kurir</h4>
              </div>
			...
			<?php 
				...
				if(!empty($booking_service)){		
					foreach($booking_service as $c){
						...
						if($c['status'] == 1){
							$_status = '<small class="label label-info"><strong>Menunggu Jadwal Kurir</strong></small>';
							$btn_action = '<button style="width:68px; margin-bottom:3px;" title="Set Kurir" id="'.$c['id_booking'].'" class="btn btn-xs btn-warning btn_set_teknisi"> Set Kurir </button><br/>';
						}
						if($c['status'] == 2){
							$_status = '<small class="label label-warning"><strong><a style="color:#ffffff;" id="'.$info_tk.'" href="#getinfo_tk">On Schedule</a></strong></small>';
							$btn_action = '<button style="width:68px; margin-bottom:3px;" id="'.$c['id_booking'].'" class="btn btn-xs btn-warning btn_confirm"> Diterima </button><br/>';
						}
						if($c['status'] == 3){
						...
						if($c['status'] == 9){
							...
							$btn_action = '<button style="width:68px; margin-bottom:3px;" id="'.$c['id_booking'].'" class="btn btn-xs btn-success btn_diterima">Paket <br/>diterima <br/>customer</button><br/>';
						}
						if($c['status'] == 10){
						...
						if($c['status'] == 13){
							$_status = '<small class="label label-info"><strong>Paket telah diterima customer</strong></small>';			
...
$('.btn_diterima').click(function(){	
	$('#frm_cat').find("input[type=text], select").val("");	
	var val = $(this).get(0).id;
	
	$('#confirm_diterima_cust').modal({
		backdrop: 'static',
		keyboard: false
	});
	$('#id_booking').val(val);
	$('#confirm_diterima_cust').modal('show');
});
...
$('.yes_diterima_cust').click(function(){
	var catatan_terima = $('#catatan_terima_cust').val();
	var id_booking = $('#id_booking').val();
	var url = '<?php echo site_url('booking_service/set_diterima_cust');?>';
	$.ajax({
		data:{id_booking:id_booking,catatan:catatan_terima},
		type:'POST',
		url : url,
		success:function(response){
			if(response > 0){
				$('#confirm_diterima_cust').modal('hide');
				$("#id_text").html('<b>Success,</b> Data telah di update');
				$("#success-alert").fadeTo(2000, 500).slideUp(500, function(){
					$("#success-alert").alert('close');
					location.reload();
