...
$disabled_antrian = ' disabled';
if((int) $antrian->nmr_antrian > 0) $disabled_antrian = '';
?>
...
<div class="modal fade" role="dialog" id="confirm_complete">
          <div class="modal-dialog" style="width:400px">
            <div class="modal-content">
              ...
              <div class="modal-footer" style="margin-top:0px;">                               
                <button type="button" class="btn btn-danger" data-dismiss="modal">Close</button>              
                <button type="button" class="btn btn-success btn_setcomplete">Complete</button>               
              </div>
            </div>
            <!-- /.modal-content -->
          </div>
          ...
          <div class="modal-dialog" style="width:400px">
            <div class="modal-content">
              ...
              <div class="modal-footer" style="margin-top:0px;">
                ...
                <button type="button" class="hide btn btn-success btn_onprogress">On Progress</button>               
              </div>
            </div>
            <!-- /.modal-content -->
          </div>
...
<div class="box-header">
    <a href="<?php echo site_url('booking_service/add_instore');?>"><button class="btn btn-success"><i class="fa fa-plus"></i> Add </button></a>
    <button class="btn btn-warning btn_call_antrian" <?php echo $disabled_antrian;?>><i class="glyphicon glyphicon-chevron-right"></i> Panggil Antrian </button>
	<div class="box-tools pull-right">
        <span style="font-size:18px;" class="badge bg-yellow info_antrian">Nomor Antrian : 0</span>
			...
			<?php 
				...
				if(!empty($booking_service)){		
					foreach($booking_service as $c){
						...
						$tgl_service = date('Y-m-d',strtotime($c['tgl_service']));
						$disabled_terima = '';
						$disabled_terima = 'disabled';
						if($now >= $tgl_service) $disabled_terima = '';
						$path_payment ='';
						$_info = '';
						$info_kurir = '';
						$path_kurir = '';
						...
						if($c['status'] == 1){
							$_status = '<small class="label label-info"><strong>Menunggu antrian</strong></small>';	
							$btn_action = '<button style="width:68px; margin-bottom:3px;" id="'.$c['id_booking'].'" class="btn btn-xs btn-success btn_confirm" '.$disabled_terima.'> Diproses </button><br/>';	
						}
						// if($c['status'] == 2){
						...
						// }
						if($c['status'] == 3){
							$_status = '<small class="label label-success"><strong>Menunggu Antrian</strong></small>';
						...
						if($c['status'] == 7){
							...
							$btn_action = '<button style="width:68px; margin-bottom:3px;" id="'.$c['id_booking'].'" class="btn btn-xs btn-danger btn_set_complete"> Done </button><br/>';
						}
						if($c['status'] == 8){
						...
						$btn_action .= '<button style="width:68px; margin-top:3px;" id="'.$c['id_booking'].'" class="btn btn-xs btn-danger btn_set_cancel"> Cancel </button><br/>';
						$total = $c['biaya_layanan'] + $c['harga_sp'];
						echo '<tr class="'.$_class.'">';
						echo '<td align="center">'.$i++.'.</td>';
						
						echo '<td>'.$c['nmr_booking'].'<br/><b>Store</b> : '.$c['nama_store'].'<br/><b>No.Antrian</b> :'.$c['nmr_antrian'].'</td>';
...
$('.btn_call_antrian').click(function(){
	var url = '<?php echo site_url('booking_service/get_antrian');?>';
	$.ajax({
		data:'',
		type:'POST',
		url : url,
		success:function(response){
			if(response > 0){
				alert('Nomor Antrian : '+response);
				$('.info_antrian').text('Nomor Antrian : '+response);							
			}else{
				$('.btn_call_antrian').prop('disabled', true);
				alert('Antrian hari ini sudah habis');
			}
		}
	})	
});
