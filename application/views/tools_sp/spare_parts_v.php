          ...
          <div class="modal-dialog" style="width:800px">
            <div class="modal-content">
              <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">×</span></button>
                <h4 class="modal-title">Add/Edit Spare Parts</h4>
              </div>
              ...
              <div class="modal-body" style="padding-bottom:2px;">
				...
				<div class="form-group">
                  <label>Spare Parts</label><span class="label label-danger pull-right name_error"></span>
                  <input type="text" class="form-control" name="tools" id="tools" value="" placeholder="Spare Parts" autocomplete="off" />
                </div>	
...
<div class='alert alert-info alert-dismissable' id="success-alert">
	...
	<table id="example88" class="table table-bordered table-striped">
		<thead><tr>
			...
			<th style="text-align:center; width:20%">Nama Spare Parts</th>	
		...
		<tbody>
			<?php 
				...
				if(!empty($tools)){		
					foreach($tools as $c){	
						$view_sub = '';
						$path = !empty($c['img']) ? base_url('uploads/spare_parts/'.$c['img']) : base_url('uploads/no_photo.jpg');
						$info = $c['id_spare_parts'].'Þ'.$c['nama'].'Þ'.$c['deskripsi'].'Þ'.number_format($c['harga'],0,'',',').'Þ'.$path;
						echo '<tr>';
			...
			<a href="#" id="'.$info.'" title="Edit" class="edit_category"><button class="btn btn-xs btn-success"><i class="fa fa-edit"></i> Edit</button></a>
				<button title="Delete" id="'.$c['id_spare_parts'].'" class="btn btn-xs btn-danger del_category" style="margin-top:5px;"><i class="fa fa-trash-o"></i> Delete</button>		
						</td>';
						echo '</tr>';
					}
				}
...
$('.yes_del').click(function(){
	var id = $('#del_id').val();
	var url = '<?php echo site_url('spare_parts/del');?>';
	$.ajax({
		data : {id : id},
		url : url,
		type : "POST",
...
$('.yes_save').click(function(){
	...
	var url = '<?php echo site_url('spare_parts/simpan_cat');?>';
